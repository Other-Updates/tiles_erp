...
//        }
    ...
    public function get_all_stock($firm_id, $filter_data = array()) {
        ...
        $this->db->select('erp_category.categoryName,erp_product.product_name,erp_brand.brands,erp_stock.quantity,erp_product.model_no,erp_manage_firms.firm_name');
...
//        }
    ...
    public function get_all_sales_id($firm_id, $filter_data = array()) {
        $this->db->select('erp_quotation.id,erp_quotation.q_no,erp_quotation.net_total,customer.store_name,erp_invoice.id as i_id,erp_invoice.net_total as inv_amount,erp_invoice.inv_id,erp_invoice.invoice_status,erp_invoice.payment_status,erp_invoice.delivery_status,erp_invoice.contract_customer,erp_invoice.customer,erp_invoice.created_date');
...
//        }
    ...
    public function get_product($input) {
        $this->db->select('*');
        $this->db->where($this->erp_product . '.status', 1);
        $this->db->where($this->erp_product . '.id', $input);
        $query = $this->db->get($this->erp_product)->result_array();
        return $query;
    }
