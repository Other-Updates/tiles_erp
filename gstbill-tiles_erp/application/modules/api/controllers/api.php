...
class Api extends MX_Controller {
    ...
    public function login() {
        ...
        if (!empty($json_input)) {
            ...
            if (!empty($data['password']) && !empty($data['username'])) {
                ...
                if ($result == 1) {
                    $output = array('status' => 'Success', 'message' => 'logged in', 'user_id' => $user_data[0]['id']);
    ...
    public function api_purchase_order_list() {
        ...
        if (!empty($json_input)) {
            ...
            if (!empty($po)) {
                $output = array('status' => 'Success', 'message' => 'Purchase Order List', 'data' => $po);
                echo json_encode($output);
            } else {
    ...
    public function api_purchase_return_list() {
        ...
        if (!empty($json_input)) {
            ...
            $pr = $this->api_model->get_all_pr($firm_id);
    ...
    public function api_purchase_receipt_list() {
        ...
        if (!empty($json_input)) {
            ...
            $prc = $this->api_model->get_all_receipt($firm_id);
    ...
    public function api_inventory_stock_list() {
        ...
        if (!empty($json_input) && $data['user_id'] != '') {
            ...
            if (!empty($stock)) {
                $output = array('status' => 'Success', 'message' => 'inventory Stock List', 'data' => $stock);
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Data Not Found');
                echo json_encode($output);
    ...
    public function api_sales_invoice_list() {
        ...
        if (!empty($json_input)) {
            ...
            if (!empty($quotation)) {
                $output = array('status' => 'Success', 'message' => 'Sales Invoice List', 'data' => $quotation, 'total_count' => $quotation_count);
                echo json_encode($output);
            } else {
                $output = array('status' => 'error', 'message' => 'Data Not Found');
                echo json_encode($output);
    ...
    public function api_get_all_firms() {
        $firms = $this->api_model->get_all_firms();
        if (!empty($firms)) {
    ...
    public function api_new_direct_invoice() {
        ...
        if (!empty($json_input)) {
            ...
            if ($input['delivery_status'] == 'delivered') {
                $invoice['delivery_qty'] = $input['total_qty'];
            }
...
//            $quotation['mode_of_payment'] = ($input['total']['mode_of_payment']) ? $input['total']['mode_of_payment'] : '-';
    ...
    public function api_add_purchase_order() {
        ...
        if (!empty($json_input)) {
            ...
            if ($quotation['pr_status'] == 'approved') {
                $this->api_model->insert_pr($quotation);
            }
    ...
    public function api_get_product_by_barcode() {
        ...
        if (!empty($json_input) && $data['barcode'] != '') {
            $product = $this->api_model->get_product_by_barcode($data['barcode']);
            if (!empty($product)) {
                $output = array('status' => 'Success', 'message' => 'Product Data', 'data' => $product);
                echo json_encode($output);
            } else {
