    ...
    function get_all_shift_datas() {
        ...
        if (count($query) > 0) {
            foreach ($query as $key => $result) {
                $result_data['shift_id'] = $result['shift_id'];
                $result_data['id'] = $result['id'];
                $result_data['day_shift'] = $result['name'];
                if ($result['type'] == "forenoon") {
                    $result_data['morning_in_time'] = $result['from_time'];
                }
                if ($result['type'] == "afternoon") {
                    $result_data['afternoon_out_time'] = $result['to_time'];
                }
            }
            return $result_data;
        } else {
