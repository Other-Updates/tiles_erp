...
class Reference_groups_model extends CI_Model {
    ...
    private $table_name = 'erp_reference_groups';
    ...
    function get_references() {
        /* $this->db->select($this->table_name . '.*');
          ...
          $sub_query .= $this->db->select('master_state.state');
          $sub_query .= $this->db->select('erp_manage_firms.firm_name');
          foreach ($query as $query_val) {
          if ($query_val['reference_type'] == 1) {
          $sub_query .= $this->db->select('erp_user.name');
          $sub_query .= $this->db->join('erp_user', 'erp_user.id=' . $this->table_name . '.user_id');
          }
          if ($query_val['reference_type'] == 2) {
          $sub_query .= $this->db->select('customer.name');
          $sub_query .= $this->db->join('customer', 'customer.id=' . $this->table_name . '.customer_id');
          }
          }
          $sub_query .= $this->db->where($this->table_name . '.status', 1);
          $sub_query .= $this->db->join('master_state', 'master_state.id=' . $this->table_name . '.state_id');
          $sub_query .= $this->db->join('erp_manage_firms', 'erp_manage_firms.firm_id=' . $this->table_name . '.firm_id');
          $return_query = $this->db->get($this->table_name)->result_array();
    ...
    function update_ref_group($data, $id) {
    ...
    function delete_reference_group($id) {
        $this->db->where('id', $id);
