...
class Purchase_order_model extends CI_Model {
    ...
    var $selectcolumn = 'v.tin,v.store_name,v.state_id,v.name,v.mobil_number,v.email_id,v.address1,p.*,f.firm_name';
...
//        }
    ...
    public function get_all_po_for_report($search = NULL) {
        ...
        if ($search != NULL && $search != '') {
            ...
            if ($search_data[2]->from != '') {
                $this->db->where($this->erp_po . '.created_date >=', $search_data[2]->from);
            }
            if ($search_data[3]->to != '') {
                $this->db->where($this->erp_po . '.created_date <=', $search_data[3]->to);
            }
        }
        $this->db->where_in('erp_po.firm_id', $frim_id);
        $query = $this->db->get('erp_po');
    ...
    public function get_all_po_details_by_id($id) {
        $this->db->select('erp_category.categoryName,erp_product.product_name,erp_brand.brands,erp_product.hsn_sac_name,erp_po_details.*');
