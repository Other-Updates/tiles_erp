...
class Gen_model extends CI_Model {
    ...
    private $erp_quotation_return = 'erp_quotation_return';
    private $erp_quotation_return_details = 'erp_quotation_return_details';
    ...
    var $selectcolumn = 'c.store_name,c.state_id,c.tin,c.name,c.mobil_number,c.email_id,c.address1,q.id as q_id,q.q_no,q.total_qty,q.tax,q.ref_name,q.tax_label,'
    ...
    public function update_payment($data,$id) {
        $this->db->where($this->increment_table . '.id', $id);
    ...
    public function get_product_by_frim_id($atten_inputs) {
        $this->db->select('erp_product.id,erp_product.product_name as value,erp_product.category_id');
        // $this->db->join('erp_stock', 'erp_stock.product_id = erp_product.id', 'left');
    ...
    public function get_product_by_frim_id_v2($atten_inputs) {
        $this->db->select('erp_product.id,erp_product.product_name as value,erp_product.category_id');
        //$this->db->join('erp_stock', 'erp_stock.product_id = erp_product.id', 'left');
        $this->db->like('erp_product.product_name', $atten_inputs['pro']);
        $this->db->where($this->erp_product . '.status', 1);
    ...
    public function get_all_quotation($serch_data) {
        ...
        $this->db->select('customer.id as customer,customer.store_name,customer.state_id,customer.tin,customer.name,customer.mobil_number,customer.email_id,customer.address1,erp_quotation.id,erp_quotation.q_no,erp_quotation.total_qty,erp_quotation.tax,erp_quotation.ref_name,erp_quotation.tax_label,'
                . 'erp_quotation.net_total,erp_quotation.taxable_price,erp_quotation.delivery_schedule,erp_quotation.notification_date,erp_quotation.mode_of_payment,erp_quotation.remarks,erp_quotation.subtotal_qty,erp_quotation.estatus,erp_quotation.validity,erp_quotation.created_date,erp_quotation_return.id as return_id');
        ...
        $this->db->join('erp_quotation_return', 'erp_quotation_return.q_id=erp_quotation.id','left');
...
//        SELECT *
//FROM (`ci_sessions`)
//WHERE `session_id` =  'fe2b392572548a61a2cabe41808a121c'
//AND `user_agent` =  'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/70.0.3538.77 Safari/537.36'
        $i = 0;
        foreach ($query as $val) {
            $this->db->select(' sum(sub_total - ( per_cost * quantity )) as tot_tax');
            $this->db->where($this->erp_quotation_details . '.q_id', $val['id']);
...
//        exit;
    ...
    public function get_all_quotation_by_id($id) {
        $this->db->select('erp_user.nick_name,customer.id as customer,customer.store_name,customer.state_id,customer.tin,customer.name,customer.mobil_number,customer.email_id,customer.address1,erp_quotation.id,erp_quotation.q_no,erp_quotation.cus_state_id,erp_quotation.total_qty,erp_quotation.tax,erp_quotation.ref_name,erp_quotation.tax_label,'
                . 'erp_quotation.net_total,erp_quotation.taxable_price,erp_quotation.cgst_price,erp_quotation.sgst_price,erp_quotation.igst_price,erp_quotation.delivery_schedule,erp_quotation.notification_date,erp_quotation.mode_of_payment,erp_quotation.remarks,erp_quotation.subtotal_qty,erp_quotation.estatus,erp_quotation.validity,erp_quotation.created_date,erp_quotation.firm_id,erp_manage_firms.firm_name');
    ...
    public function get_all_quotation_details_by_id($id) {
        $this->db->select('erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,erp_product.model_no,'
                ...
                . 'erp_quotation_details.product_description,erp_product.type,erp_product.hsn_sac_name,erp_product.sgst as pro_sgst,erp_product.igst as pro_igst');
...
//        }
    ...
    public function get_all_quotation_history_by_id($id) {
        $this->db->select('erp_user.nick_name,customer.store_name,customer.state_id,customer.tin,customer.id,customer.name,customer.mobil_number,customer.email_id,customer.address1,erp_quotation_history.q_no,erp_quotation_history.total_qty,erp_quotation_history.tax,erp_quotation_history.ref_name,erp_quotation_history.tax_label,'
                . 'erp_quotation_history.net_total,erp_quotation_history.cus_state_id,erp_quotation_history.taxable_price,erp_quotation_history.cgst_price,erp_quotation_history.sgst_price,erp_quotation_history.igst_price,erp_quotation_history.delivery_schedule,erp_quotation_history.notification_date,erp_quotation_history.mode_of_payment,erp_quotation_history.remarks,erp_quotation_history.subtotal_qty,erp_quotation_history.validity,erp_quotation_history.created_date,erp_quotation_history.firm_id,erp_manage_firms.firm_name');
    ...
    public function get_all_quotation_history_details_by_id($id) {
        $this->db->select('erp_category.cat_id,erp_category.categoryName,erp_brand.id,erp_brand.brands,erp_product.model_no,'
                ...
                . 'erp_quotation_history_details.per_cost,erp_quotation_history_details.discount,erp_quotation_history_details.tax,erp_quotation_history_details.gst,erp_quotation_history_details.sub_total,erp_product.hsn_sac_name');
        $this->db->where('erp_quotation_history.id', $id);
    ...
    public function check_quotation_exists($quotation_no){
		$exists = 0;
		$this->db->select($this->erp_quotation.'.id');
		$this->db->where($this->erp_quotation.'.estatus',1);
		$this->db->where($this->erp_quotation.'.q_no',$quotation_no);
		$query = $this->db->get($this->erp_quotation)->result_array();
		if(count($query) > 0)
			$exists = 1;
    ...
    function get_quotation_return_datatables($search_data) {

        $this->db->select('erp_quotation_return.id as return_id,erp_quotation_return.total_qty as return_quantity,erp_quotation_return.net_total as return_amt,erp_quotation.*,customer.store_name as customer_name');
        $this->db->join('erp_quotation', 'erp_quotation_return.q_id=erp_quotation.id');
        $this->db->join('customer', 'customer.id=erp_quotation.customer');
        $this->db->order_by('erp_quotation.id', 'desc');
        $query = $this->db->get('erp_quotation_return')->result_array();

        // foreach ($query as $key => $data) {
        //     $this->db->select('SUM(return_quantity) as return_quantity,SUM(sub_total) as return_amt');
        //     $this->db->where('q_id', $data['id']);
        //     $this->db->where('return_quantity !=', 0);
        //     $get_return_details = $this->db->get('erp_quotation_return_details')->result_array();
    ...
    public function insert_quotation_return($data) {
        if ($this->db->insert($this->erp_quotation_return, $data)) {
            $insert_id = $this->db->insert_id();
    ...
    public function insert_quotation_return_details($data) {
        $this->db->insert_batch($this->erp_quotation_return_details, $data);
        return true;
    }

    public function get_all_return_by_id($id) {
        $this->db->select('customer.id as customer,customer.store_name,customer.state_id,customer.tin,customer.name,customer.mobil_number,customer.email_id,customer.address1,erp_quotation_return.id,erp_quotation_return.quotation as q_no,erp_quotation_return.total_qty,erp_quotation_return.tax,erp_quotation_return.tax_label,'
                . 'erp_quotation_return.net_total,erp_quotation_return.cus_state_id,erp_quotation_return.taxable_price,erp_quotation_return.cgst_price,erp_quotation_return.sgst_price,erp_quotation_return.igst_price,erp_quotation_return.subtotal_qty,erp_quotation_return.created_date,erp_quotation_return.firm_id,erp_manage_firms.firm_name');
        //$this->db->where('erp_quotation.estatus',1);
        $this->db->where('erp_quotation_return.id', $id);
        $this->db->join('customer', 'customer.id=erp_quotation_return.customer');
        $this->db->join('erp_manage_firms', 'erp_manage_firms.firm_id=erp_quotation_return.firm_id');
        $query = $this->db->get('erp_quotation_return');
        if ($query->num_rows() >= 0) {
            return $query->result_array();
        }
    ...
    public function get_all_return_details_by_id($id) {
        $this->db->select('erp_category.cat_id,erp_category.categoryName,erp_product.id,erp_product.product_name,erp_brand.id,erp_brand.brands,erp_product.model_no,'
                . 'erp_quotation_return_details.id as del_id,erp_quotation_return_details.category,erp_quotation_return_details.product_id,erp_quotation_return_details.brand,erp_quotation_return_details.return_quantity as quantity,'
                . 'erp_quotation_return_details.per_cost,erp_quotation_return_details.tax,erp_quotation_return_details.gst,erp_quotation_return_details.discount,erp_quotation_return_details.sub_total,erp_product.model_no,erp_product.product_image,erp_quotation_return_details.unit,erp_quotation_return_details.igst,'
                . 'erp_quotation_return_details.product_description,erp_product.type,erp_product.hsn_sac_name');
        $this->db->where('erp_quotation_return_details.return_id', $id);
        $this->db->join('erp_quotation_return', 'erp_quotation_return.id=erp_quotation_return_details.return_id');
        $this->db->join('erp_category', 'erp_category.cat_id=erp_quotation_return_details.category', 'LEFT');
        $this->db->join('erp_product', 'erp_product.id=erp_quotation_return_details.product_id');
        $this->db->join('erp_brand', 'erp_brand.id=erp_quotation_return_details.brand', 'LEFT');

        $query = $this->db->get('erp_quotation_return_details')->result_array();
