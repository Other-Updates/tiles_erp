...
class Quotation extends MX_Controller {
    ...
    function __construct() {
        ...
        $access_arr = array(
            ...
            'quotation/quotation_return' => array('add', 'edit', 'delete', 'view'),
            'quotation/quotationreturn' => array('add', 'edit'),
            'quotation/quotationreturnview' => 'no_restriction',
            ...
            'quotation/check_quotation_exists' => 'no_restriction',
            'quotation/make_quotation_return_ajax' => 'no_restriction', 
    ...
    public function index() {
        if ($this->input->post()) {
            ...
            if($input['quotation']['customer'] == ''){
                $customer_arr['state_id'] = $input['quotation']['cus_state_id'];
                $customer_arr['customer_type'] = '1';
                $customer_arr['firm_id'] = $input['quotation']['firm_id'];
                $customer_arr['tin'] = $customer_arr['tin_no'];
                unset($customer_arr['tin_no']);    
                $insert_customer = $this->customer_model->insert_customer($customer_arr);
                if (isset($insert_customer) && !empty($insert_customer)) {
                    $input['quotation']['customer'] =  $insert_customer;
    ...
    public function check_quotation_exists(){
		$quotation_no = $this->input->post('grn_no');
		$result["result"] = $this->gen_model->check_quotation_exists($quotation_no);
...
//$contract_customer = $input['quotation']['contract_customer'];
    ...
    function quotation_ajaxList() {
        ...
        foreach ($list as $ass) {
            ...
            $row[] = number_format($ass['net_total'] - $ass['taxable_price'], 2);
            $row[] = number_format($ass['net_total'], 2);
            $row[] = (($ass['created_date'] != '1970-01-01') ? date('d-M-Y', strtotime($ass['created_date'])) : '');
            if ($ass['estatus'] == '1') {
                $qutotation_status = '<span class="badge bg-red">Pending</span>';
            } else if ($ass['estatus'] == '2') {
                $qutotation_status = '<span class=" badge bg-green">Completed</span>';
            } else if ($ass['estatus'] == '4') {
                $qutotation_status = '<span class=" badge bg-green">Order Approved</span>';
            } else if ($ass['estatus'] == '5') {
                $qutotation_status = '<span class=" badge bg-green">Order Reject</span>';
            }
            $row[] = $qutotation_status;

            if ($ass['estatus'] == '2') {
                if ($this->user_auth->is_action_allowed('quotation', 'quotation', 'view')) {
                    $rows = '<a href="' . $this->config->item('base_url') . 'quotation/quotation_view/' . $ass['q_id'] . '" data-toggle="tooltip" class="tooltips btn btn-default btn-xs" title="View" ><span class="fa fa-log-out "> <span class="fa fa-eye"></span>  </span></a>&nbsp;&nbsp;';
                } else {
                    $rows = '<a href="#" data-toggle="tooltip" class="btn btn-default btn-xs alerts" title="View" ><span class="fa fa-log-out "> <span class="fa fa-eye"></span>  </span></a>&nbsp;&nbsp;';
                }
                if($ass['return_id'] == '')
                    $rows .= '<a href="' . $this->config->item('base_url') . 'quotation/quotationreturn/' . $ass['q_id'] . '" data-toggle="tooltip" class="btn btn-default btn-xs" title="Make Return" data-original-title="Make Return"><i class="fa fa-reply-all"></i></a>';
            } else {
                $user_info = $this->user_info = $this->user_auth->get_from_session('user_info');

                if (($user_info[0]['role'] != 3)) {
                    if ($this->user_auth->is_action_allowed('quotation', 'quotation', 'edit')) {
                        $rows = '<a href="' . $this->config->item('base_url') . 'quotation/quotation_edit/' . $ass['q_id'] . '" data-toggle="tooltip" class="tooltips btn btn-default btn-xs" title="Edit" data-original-title="Edit" ><span class="fa fa-edit "></span></a>&nbsp;&nbsp;';
                    } else {
                        $rows = '<a href="#" data-toggle="tooltip" class="btn btn-default btn-xs alerts" title="Edit" data-original-title="Edit" ><span class="fa fa-edit "></span></a>&nbsp;&nbsp;';
                    }
                    if ($this->user_auth->is_action_allowed('quotation', 'quotation', 'delete')) {
                        $rows .=' <a href="#test3_'. $ass['q_id'] . '" data-toggle="modal" name="delete" class="btn btn-default btn-xs" title="In-Active"><span class="fa fa-ban "></span></a>&nbsp;&nbsp;';
                    } else {
                        $rows .= ' <a href="#" data-toggle="modal" name="delete" class="btn btn-default btn-xs alerts" title="In-Active"><span class="fa fa-ban "></span></a>&nbsp;&nbsp;';
                    }
                }
                if ($this->user_auth->is_action_allowed('quotation', 'quotation', 'view')) {
                    $rows .=' <a href="' . $this->config->item('base_url') . 'quotation/quotation_view/' . $ass['q_id'] . '" data-toggle="tooltip" class="tooltips btn btn-default btn-xs" title="View" ><span class="fa fa-log-out "> <span class="fa fa-eye"></span>  </span></a>&nbsp;&nbsp;';
...
//        echo '</ul>';
    ...
    public function get_product_by_frim_id() {
        ...
        $input = $this->input->post();
//        $products = $this->gen_model->get_product_by_frim_id($input['firm_id']);
        $products = $this->gen_model->get_product_by_frim_id_v2($input);
    ...
    public function get_product($id) {
        $atten_inputs = $this->input->post();
        $product_data = $this->gen_model->get_product($atten_inputs, $id);
    ...
    public function quotation_edit($id) {
        ...
        $this->load->model('customer/customer_model');
        $datas['all_state']=$this->customer_model->state();
    ...
    function export_csv($query, $timezones = array()) {
        ...
        foreach ($query as $val) {
            ...
            $row = array($val['q_no'], $val['name'], $val['total_qty'], $val['net_total'], $val['delivery_schedule'], $val['created_date']);
            fputcsv($output, $row);
        }
        exit;
    }
    ...
    function quotation_return(){
        $datas['quotation'] = $this->gen_model->get_all_quotation();
        $datas['company_details'] = $this->admin_model->get_company_details();
        $this->template->write_view('content', 'quotation/quotation_return_list', $datas);
        $this->template->render();
    }

    public function make_quotation_return_ajax() {
        $search_data = $this->input->post();
        $search_arr = array();
        $list = $this->gen_model->get_quotation_return_datatables($search_arr);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $val) {
            $no++;
            ...
            $row[] = number_format($val['total_qty']);
            $row[] = number_format($val['net_total'], 2);
            $row[] = number_format($val['return_quantity']);
            $row[] = number_format($val['return_amt'], 2);
            $url = $this->config->item('base_url') . 'quotation/quotationreturnview/'.$val['return_id'];
            $row[] = '<a class="tooltips btn btn-default back btn-xs" data-toggle="tooltip" data-original-title="View" href="' . $url . '"  data-placement="top" title="View"><span class="fa fa-eye"></span></a>';
    ...
    public function quotationreturn($quotation_id){
        if ($this->input->post()) {
            $input = $this->input->post();
            $data['company_details'] = $this->admin_model->get_company_details();
            $insert_id = $this->gen_model->insert_quotation_return($input['quotation']);
            
            $q_no = $input['quotation']['q_no'];
            $q_id = $input['quotation']['q_id'];
            if (isset($insert_id) && !empty($insert_id)) {
                $input = $this->input->post();
                if (isset($input['category']) && !empty($input['category'])) {
                    $insert_arr = array();
                    foreach ($input['category'] as $key => $val) {
                        if($input['return_quantity'][$key] > 0 ){
                            $insert['return_id'] = $insert_id;
                            $insert['q_id'] = $q_id;
                            $insert['q_detail_id'] =  $input['q_detail_id'][$key];
                            $insert['category'] = $val;
                            $insert['product_id'] = $input['product_id'][$key];
                            if (!empty($input['quotation']['product_description']))
                                $insert['product_description'] = $input['product_description'][$key];
                    ...
                    $this->gen_model->insert_quotation_return_details($insert_arr);
                }
            }
        ...
        $datas["quotation"] = $quotation = $this->gen_model->get_all_quotation_by_id($quotation_id);
        $datas["quotation_details"] = $quotation_details = $this->gen_model->get_all_quotation_details_by_id($quotation_id);
        $datas['company_details'] = $this->admin_model->get_company_details();
        $this->template->write_view('content', 'quotation_return', $datas);
        $this->template->render();
    }

    public function quotationreturnview($id) {
        $datas["return"]  = $this->gen_model->get_all_return_by_id($id);
        $datas["return_details"]  = $this->gen_model->get_all_return_details_by_id($id);
        $datas['company_details'] = $this->admin_model->get_company_details();
        $this->template->write_view('content', 'quotation_return_view', $datas);
        $this->template->render();
    }
